...
class LogsTest(BaseCombinedDbTest):
    ...
    articles = [
        ...
        (b"Test results", b"GA-Class", b"NotA-Class", b"Mid-Class", b"NotA-Class"),
        ...
        (b"Testing figures", b"NotA-Class", b"A-Class", b"NotA-Class", b"Low-Class"),
        (
            b"Important tests",
            b"NotA-Class",
            b"NotA-Class",
        ...
        (b"How to test", b"Stub-Class", b"C-Class", b"NotA-Class", b"Unknown-Class"),
    ...
    def _insert_moves(self):
        move_data = [
            (0, m[0], 0, m[1], self.wiki_timestamps[i % len(self.wiki_timestamps)])
            for i, m in enumerate(self.moves)
        ]
        with self.wp10db.cursor() as cursor:
            cursor.executemany(
    ...
    def test_get_revid_nonexistant_page(self):
        actual = logs.get_revid(self.wikidb, b"Non-existant", 0, datetime(2018, 1, 1))
    ...
    def test_get_revid(self):
        for l in self._logs():
            if l.l_action == b"moved":
                continue
            actual = logs.get_revid(
                self.wikidb, l.l_article, l.l_namespace, l.rev_timestamp_dt
            )
    ...
    def test_calculate_logs_to_update_keys(self, patched_datetime_now):
        actual = logs.calculate_logs_to_update(self.wikidb, self.wp10db, self.project)
    ...
    def test_calculate_logs_to_update_values(self, patched_current_datetime):
        expected = [
            l for l in self._logs() if l.l_revision_timestamp.startswith(b"2018-12-25")
        ]
        expected.extend(
            l
            for l in self._move_logs()
        ...
        for d in ((25, b"2018-12-25"), (26, b"2018-12-26"), (27, b"2018-12-27")):
            ...
            expected.extend(
                l for l in self._move_logs() if l.l_revision_timestamp.startswith(d[1])
            )
            self.assertEqual(
                sorted(expected), sorted(actual[datetime(2018, 12, d[0]).date()])
            )

    def test_get_section_categories(self):
        expected = {
            "assessed": {b"Testing mechanics", b"Test frameworks", b"Important tests"},
            "reassessed": {
                b"Art of testing",
                b"Failures of tests",
                b"How to test",
    ...
    def test_get_section_data(self):
        expected = {
            ...
            "revid": {
                b"Important tests": {"importance": 15000, "quality": 15000},
                b"Lesser-known tests": {"importance": 18000, "quality": 18000},
                b"Test other inheritance": {"quality": 22000},
                b"Test practices": {"quality": 14000},
                b"Testing None-a": {"importance": 14000},
                b"Testing history": {"quality": 15000},
                b"Testing tools": {"quality": 13000},
            },
            "talk": {
            ...
            "talk_revid": {
                b"Important tests": {"importance": None, "quality": None},
                b"Lesser-known tests": {"importance": None, "quality": None},
                b"Test other inheritance": {"quality": None},
                b"Test practices": {"quality": None},
                b"Testing None-a": {"importance": None},
                b"Testing history": {"quality": None},
                b"Testing tools": {"quality": None},
            },
        }
        test_logs = [
            l for l in self._logs() if l.l_revision_timestamp.startswith(b"2018-12-25")
    ...
    @patch("wp1.logs.api")
    def test_upload_log_page_for_project(self, patched_api, patched_wp10, patched_wiki):
    ...
    @patch("wp1.logs.api")
    @patch("wp1.logs.get_current_datetime", return_value=datetime(2018, 12, 28, 12))
    def test_upload_log_page_for_project_no_logs(
        self, patched_datetime, patched_api, patched_wp10, patched_wiki
    ):
    ...
    def test_upload_log_page_for_project_huge_text(
        self, patched_generate, patched_api, patched_wp10, patched_wiki
    ):
        project_name = b"Catholicism"
        header = (
            "{{Log}}\n"
    ...
    @patch("wp1.logs.api")
    @patch("wp1.logs.generate_log_edits")
    def test_upload_log_page_for_project_huge_give_up(
        self, patched_generate, patched_api, patched_wp10, patched_wiki
    ):
        project_name = b"Catholicism"
