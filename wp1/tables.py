...
def get_project_categories(wp10db, project_name):
    ...
    for row in categories:
        if row["c_rating"] == NOT_A_CLASS:
            ...
            labels[row["c_rating"]] = "{{%s|category=Category:%s}}" % (
...
def generate_table_data(stats, categories, table_overrides=None):
    ...
    ordered_cols = sorted(
        cols.keys(), key=lambda x: categories["sort_imp"][x], reverse=True
    )
    ordered_rows = sorted(
        data.keys(), key=lambda x: categories["sort_qual"][x], reverse=True
    )
    if UNASSESSED_CLASS in ordered_rows:
        idx = ordered_rows.index(UNASSESSED_CLASS)
        ordered_rows = ordered_rows[:idx] + [b"Assessed-Class"] + ordered_rows[idx:]
    else:
...
def upload_project_table(project_name):
    ...
    try:
        logger.info("Getting table data for project: %s", project_name.decode("utf-8"))
        ...
        logger.info("Uploading wikicode to Wikipedia: %s", project_name.decode("utf-8"))
