...
def update_project_by_name(project_name, track_progress=False):
    ...
    try:
        ...
        if not project:
            project = Project(p_project=project_name, p_timestamp=GLOBAL_TIMESTAMP_WIKI)
        update_project(
            wikidb, wp10db, project, redis=redis, track_progress=track_progress
        )
...
def project_names_to_update(wikidb):
    ...
    for category_page in list(projects_in_root):
        if BY_QUALITY not in category_page.page_title:
            print(
                "Skipping %s -- it does not have quality in title"
        ...
        if RE_REJECT_GENERIC.match(category_page.page_title):
            print(
                'Skipping %r -- it is a generic "articles by quality"' % category_page
            )
            continue
...
def get_project_by_name(wp10db, project_name):
    with wp10db.cursor() as cursor:
        cursor.execute(
            "SELECT * FROM projects WHERE p_project=%(p_project)s",
...
def update_category(wp10db, project, page, extra, kind, rating_to_category):
    ...
    if extra_category is not None:
        ...
        if not md:
            logger.debug(
                "Skipping page with no class match: %s", page.page_title.decode("utf-8")
            )
            return
        ...
        if rating not in rating_map:
            logger.debug(
                "Skipping page with indicator not in mapping: %s | %s",
...
def count_initial_work(redis, wp10db, project_name):
    if redis is None:
        logger.error("Attempt to track progress without specifying a Redis instance")
        return
...
def update_project_assessments(
    wikidb, wp10db, project, extra_assessments, redis=None, track_progress=False
):
    old_ratings = {}
    for rating in logic_rating.get_project_ratings(wp10db, project.p_project):
        rating_ref = str(rating.r_namespace).encode("utf-8") + b":" + rating.r_article
    ...
    for current_rating, (category, ranking) in rating_to_category.items():
        ...
        for page in logic_page.get_pages_by_category(wikidb, category):
            ...
            if not logic_util.is_namespace_acceptable(namespace):
                logger.debug(
                    "Skipping %s with namespace=%s", page.page_title, namespace
                )
                continue
...
def store_new_ratings(wp10db, new_ratings, old_ratings, rating_to_category):
    def sort_rating_tuples(rating_tuple):
        ...
        if kind == AssessmentKind.QUALITY:
            # ranking
            return rating_to_category[rating.r_quality.decode("utf-8")][1]
        if kind == AssessmentKind.IMPORTANCE:
            return rating_to_category[rating.r_importance.decode("utf-8")][1]  # ranking
...
def cleanup_project(wp10db, project):
    ...
    logger.info(
        "Deleted %s ratings that were empty for project: %s",
        count,
    ...
    logger.info(
        "Updated %s ratings, quality == NotAClass for project: %s",
        count,
        project.p_project.decode("utf-8"),
    )
    ...
    logger.info(
        "Updated %s ratings, importance == NotAClass for project: %s",
        count,
        project.p_project.decode("utf-8"),
    )
...
def update_project_record(wp10db, project, metadata):
    ...
    n_importance = logic_rating.count_unassessed_importance_for_project(wp10db, project)
