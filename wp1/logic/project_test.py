...
class UpdateCategoryTest(BaseWpOneDbTest):
    def setUp(self):
        ...
        self.page = Page(
            page_title=b"A-Class Test articles", page_id=None, page_namespace=None
        )
        self.page_1 = Page(
            page_title=b"Mid-importance Test articles",
            page_id=None,
            page_namespace=None,
        )
        self.page_2 = Page(
            page_title=b"Draft-Class Test articles", page_id=None, page_namespace=None
        )
    ...
    def test_extra_category_gets_updated(self):
        ...
        category = _get_first_category(self.wp10db)
        self.assertEqual(self.page_2.page_title, rating_to_category["Draft-Class"][0])
        self.assertEqual(self.project.p_project, category.c_project)
        self.assertEqual(b"quality", category.c_type)
...
class UpdateProjectCategoriesByKindTest(BaseCombinedDbTest):
    quality_pages = (
        (101, b"FA-Class_Test_articles", b"Test_articles_by_quality", b"FA-Class", 500),
        (102, b"FL-Class_Test_articles", b"Test_articles_by_quality", b"FL-Class", 480),
        (103, b"A-Class_Test_articles", b"Test_articles_by_quality", b"A-Class", 425),
        (104, b"GA-Class_Test_articles", b"Test_articles_by_quality", b"GA-Class", 400),
        (105, b"B-Class_Test_articles", b"Test_articles_by_quality", b"B-Class", 300),
        (106, b"C-Class_Test_articles", b"Test_articles_by_quality", b"C-Class", 225),
    )
    ...
    additional_junk_pages = (
        ...
        (203, b"Mid-Class_Foo_articles", b"Foo_articles_by_importance", b"Mid-Class"),
        (204, b"Low-Class_Bar_articles", b"Bar_articles_by_importance", b"Low-Class"),
    )

    importance_pages = (
        (101, b"Top-Class_Test_articles", b"Test_articles_by_importance", b"Top-Class"),
        (
            102,
            b"High-Class_Test_articles",
            b"Test_articles_by_importance",
            b"High-Class",
        ),
        (103, b"Mid-Class_Test_articles", b"Test_articles_by_importance", b"Mid-Class"),
        (104, b"Low-Class_Test_articles", b"Test_articles_by_importance", b"Low-Class"),
    )

    priority_pages = (
        (101, b"Top-Class_Test_articles", b"Test_articles_by_priority", b"Top-Class"),
        (102, b"High-Class_Test_articles", b"Test_articles_by_priority", b"High-Class"),
        (103, b"Mid-Class_Test_articles", b"Test_articles_by_priority", b"Mid-Class"),
        (104, b"Low-Class_Test_articles", b"Test_articles_by_priority", b"Low-Class"),
    )
...
class ArticlesTest(BaseCombinedDbTest):
    ...
    custom_quality_pages = (
        (107, b"Draft-Class_Test_articles", b"Test_articles_by_quality", None, 14),
        (270, b"Starting out testing", b"Draft-Class_Test_articles", b"Draft-Class", 1),
        (271, b"Your First Test", b"Draft-Class_Test_articles", b"Draft-Class", 1),
    )
    ...
    quality_pages = (
        ...
        (232, b"Test other inheritance", b"GA-Class_Test_articles", b"GA-Class", 1),
    ...
    importance_pages = (
        (1010, b"Top-Class_Test_articles", b"Test_articles_by_importance", None, 14),
        (1020, b"High-Class_Test_articles", b"Test_articles_by_importance", None, 14),
        (1030, b"Mid-Class_Test_articles", b"Test_articles_by_importance", None, 14),
        (1040, b"Low-Class_Test_articles", b"Test_articles_by_importance", None, 14),
        ...
        (2300, b"Test main inheritance", b"Mid-Class_Test_articles", b"Mid-Class", 1),
        (2310, b"Test sub inheritance", b"Mid-Class_Test_articles", b"Mid-Class", 1),
        (2320, b"Test other inheritance", b"Mid-Class_Test_articles", b"Mid-Class", 1),
        (2400, b"Testing best practices", b"Low-Class_Test_articles", b"Low-Class", 1),
    ...
    multiple_importance_pages = (
        (2620, b"Operation of tests", b"Mid-Class_Test_articles", b"Mid-Class", 1),
        (2600, b"Lesser-known tests", b"Mid-Class_Test_articles", b"Mid-Class", 1),
...
class UpdateProjectAssessmentsTest(ArticlesTest):
    ...
    def test_custom_rating(self):
        ...
        with patch("wp1.logic.rating.GLOBAL_TIMESTAMP", expected_global_ts):
            logic_project.update_project_assessments(
                self.wikidb, self.wp10db, self.project, extra
            )
        ...
        for title in expected_titles:
            self.assertTrue(
                title in actual_titles, "%s not in %s" % (title, actual_titles)
            )
    ...
    def _assert_updated_quality_ratings(self, assert_log_len=True):
        ...
        q_page_to_rating = dict((p[1], p[3]) for p in q_pages)
        q_page_to_rating.update(dict((p[1], p[3]) for p in self.multiple_quality_pages))
        for r in ratings:
            self.assertEqual(q_page_to_rating[r.r_article], r.r_quality)
    ...
    def test_multiple_existing_importance(self):
        ...
        self._insert_pages(self.multiple_importance_pages)
        self._insert_ratings(
            self.multiple_importance_pages, 0, AssessmentKind.IMPORTANCE
        )
    ...
    def test_not_seen_null_quality(self, patched_site):
        self._insert_pages(self.quality_pages)
        self._insert_ratings(
            self.quality_pages[6:], 0, AssessmentKind.QUALITY, override_rating=None
        )
...
class GlobalArticlesTest(ArticlesTest):
    def setUp(self):
        ...
        self._insert_pages(self.importance_pages)
        self._insert_ratings(
            zip(self.quality_pages[6:], self.importance_pages[4:]), 0, "both"
        )
    ...
    def setUp(self):
        ...
        with self.wp10db.cursor() as cursor:
            for r in self.ratings:
                rating = Rating(
                    r_project=self.project.p_project,
                    r_namespace=0,
                    r_article=r[0],
                    r_quality=r[1],
                    r_importance=r[2],
...
class UpdateProjectRecordTest(BaseWpOneDbTest):
    ...
    def setUp(self):
        ...
        logic_project.update_project_record(self.wp10db, self.project, self.metadata)

    def test_metadata_correct(self):
        self.assertEqual(
            self.metadata["homepage"], self.project.wikipage.decode("utf-8")
        )
        self.assertEqual(
            self.metadata["shortname"], self.project.shortname.decode("utf-8")
        )
        self.assertEqual(self.metadata["parent"], self.project.parent.decode("utf-8"))
...
class UpdateProjectByNameTest(BaseCombinedDbTest):
    ...
    def test_calls_update_project_with_existing(
        self, patched_update_project, patched_wp10_connect, patched_wiki_connect
    ):
