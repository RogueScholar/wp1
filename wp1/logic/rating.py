...
def _project_rating_query(
    project_name, quality=None, importance=None, page=None, count=False
):
    if count:
        query = "SELECT COUNT(*) as count FROM " + Rating.table_name
    else:
        query = (
            "SELECT r_project, r_namespace, r_article, r_score, r_quality,"
    ...
    if quality is None:
        query += (
            ' JOIN global_rankings grq ON grq.gr_type = "quality" AND'
...
def get_project_rating_count_by_type(
    wp10db, project_name, quality=None, importance=None
):
    query = _project_rating_query(
        project_name, quality=quality, importance=importance, count=True
    )
    with wp10db.cursor() as cursor:
        cursor.execute(
            query,
...
def get_project_rating_by_type(
    wp10db, project_name, quality=None, importance=None, page=None
):
    query = _project_rating_query(
        project_name, quality=quality, importance=importance, page=page
    )
    with wp10db.cursor() as cursor:
        cursor.execute(
            query,
...
def add_log_for_rating(wp10db, new_rating, kind, old_rating_value):
    ...
    log = Log(
        l_project=new_rating.r_project,
